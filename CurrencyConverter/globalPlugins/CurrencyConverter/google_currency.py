...
def convert(currency_from, currency_to, amnt, replace_commas=True):
    ...
    url = "https://api.exchangerate-api.com/v4/latest/{}".format(currency_from)
    ...
    try:
        ...
        # Just to check whether this currency exists in our currency code base or not
        ...
        if rate:
            converted_currency = (amnt * rate)
            if replace_commas:
                converted_currency = str(converted_currency).replace(',', '')
            default_response["amount"] = converted_currency
            default_response["converted"] = True
            return converted_currency
        else:
            raise Exception("Unable to convert currency, rate not available")
